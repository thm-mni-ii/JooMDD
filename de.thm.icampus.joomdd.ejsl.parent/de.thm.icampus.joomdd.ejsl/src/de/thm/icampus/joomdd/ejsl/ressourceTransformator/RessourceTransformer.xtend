...
class RessourceTransformer {
    ...
    def completeDetailsPage() {
        ...
        for (Page page : feature.pages.filter [ t |
            t instanceof DetailsPage && (t as DetailsPage).tablecolumns.empty && (t as DetailsPage).editfields.empty
        ]) {
        ...
        for (Entity ent : entitiesList) {
            ...
            for (Reference ref : ent.references) {
                ...
                if (ref.upper.equalsIgnoreCase("-1") && referenced !== null && referenced.upper.equalsIgnoreCase("1")) {
                    ...
                    if (primaryKey !== null) {
                        getAttributeList.addAll( this.searchUniqueAttribute(ent).filter [ t |
