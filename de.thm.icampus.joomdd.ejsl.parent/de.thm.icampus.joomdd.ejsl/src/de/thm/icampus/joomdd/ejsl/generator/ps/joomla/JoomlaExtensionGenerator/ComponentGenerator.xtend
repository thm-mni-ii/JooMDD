...
public class ComponentGenerator extends AbstractExtensionGenerator {
	...
	new (ExtendedComponent component, IFileSystemAccess2 fsa, String path, String updatePath) {
		...
		this.sitePath = newArrayList(path, "components", this.extendedComp.extensionName.toLowerCase).join("/")
		this.adminPath = newArrayList(path, "administrator", "components", this.extendedComp.extensionName.toLowerCase).join("/")
		this.mediaPath = newArrayList(path, "media", extendedComp.extensionName.toLowerCase).join("/")
	}
