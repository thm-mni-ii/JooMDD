...
class DetailsPageTemplate extends DynamicPageTemplate {
    ...
    def void generateFrontEndEditView(String editPagename) {
        generateFile(viewPath + "/" + editPagename.toFirstUpper + "/" + "HtmlView.php",
            generateSiteView(true, editPagename))
        generateFile(tmplPath + "/" + editPagename + "/" + "default.php", generateSiteViewLayoutEdit(editPagename))
        generateFile(tmplPath + "/" + editPagename + "/" + "default.xml",
            xmlSiteTemplateContent(editPagename, dpage, com))
    }
    ...
    def void generateController() {
        if (sec.equalsIgnoreCase("admin")) {
            ...
            generateFile(controllerPath + "/" + pagename.toFirstUpper + "edit" + "Controller.php",
                generateSiteController(true))
    ...
    def void generateModel() {
        if (sec.equalsIgnoreCase("admin")) {
            ...
            generateFile(formPath + "/" + dpage.extendedEntityList.get(0).name.toLowerCase + ".xml",
                xmlAdminFields(dpage, com, com.name))
        } else {
            generateFile(formPath + "/" + dpage.extendedEntityList.get(0).name.toLowerCase + ".xml",
                xmlSiteFields(dpage, com, com.name))

            if (!dpage.extendedTableColumnList.empty && !dpage.extendedEditedFieldsList.isEmpty) {
                generateFile(modelPath + "/" + pagename.toFirstUpper + "edit" + "Model.php",
                    generateSiteModelEdit(pagename + "edit"))
    ...
    def CharSequence generateAdminController() '''
        ...
        class «dpage.name.toFirstUpper»Controller extends FormController
        {
             ...
             * @since  1.6
             */
            protected $option = "«Slug.nameExtensionBind("com",com.name)»";
             ...
             * @since  1.6
             */
            protected $view_item = "«dpage.name»";
             ...
             * @since  1.6
             */
            protected $view_list = "«(dpage.links.findFirst[l | l instanceof InternalLink] as InternalLink).target.name»";
        }
    '''
    ...
    def CharSequence generateModelLoadFormDataFunction() '''	
        ...
        protected function loadFormData()
        {
            ...
            $data = Factory::getApplication()->getUserState('«Slug.nameExtensionBind("com",com.name)».edit.«dpage.extendedEntityList.get(0).name».data', array());
