...
```
Pages require at least one entity as reference, which will then be represented in the view. If desired, the table `representationColumns` of the view can be specified. However, this step is optional - If no columns specified, all entity attributes will be shown as columns. If required, you can specify `filters` for the fields, which later can be used as part of the Search Tools (filters).
